...
namespace lab10
{
    class Pathfinder
    {
        ...
        /// get the gweight of nodes for update purpose
        public void updateGWeights(Level map)
        {
            for (int x = 0; x < levelWidth; x++)
            {
                for (int y = 0; y < levelHeight; y++)
                {
                    searchNodes[x, y].GWeight = map.GetTerrainCost(x, y);
                }
            }
        }
        /// Splits our level up into a grid of nodes.
        private void InitializeSearchNodes(Level map)
        {
            ...
            for (int x = 0; x < levelWidth; x++)
            {
                ...
                for (int y = 0; y < levelHeight; y++)
                {
                    ...
                    node.GWeight = map.GetTerrainCost(x, y);
        ...
        public List<Vector2> FindPath(Point startPoint, Point endPoint)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                for (int i = 0; i < currentNode.Neighbors.Length; i++)
                {
                    ...
                    //      be walked across. 
                    //////////////////////////////////////////////////
                    if (neighbor == null || neighbor.GWeight <= 0)
                    {
                        continue;
                    }
                    ...
                    //////////////////////////////////////////////////
                    float distanceTraveled = currentNode.DistanceTraveled + neighbor.GWeight;
