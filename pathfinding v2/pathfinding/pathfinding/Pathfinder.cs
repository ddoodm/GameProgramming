...
namespace pathfinding
{
    class Pathfinder
    {
        ...
        //debug draw method
        public void Draw(SpriteBatch batch, Texture2D texture, Level level)
        {
            for (int x = 0; x < searchNodes.GetLength(1); x++)
            {
                for (int y = 0; y < searchNodes.GetLength(0); y++)
                {
                    int textureIndex = level.GetIndex(x, y);
                    if (textureIndex == 0)
                        continue;
